  ...
  
{
  ...
  save(set,file=paste0(settings$Analyse_Datasets,"/2. Proxy validatie/All_Asset_NOR_Info_Validatie.Rda"))
  ...
  setorder(assets$MSkabels,DateLength_ch,na.last=TRUE)
  setkey(assets$LSkabels,ID_NAN)
  assets$LSkabels=unique(LSassets$kabels)
  assets$LSmoffen=unique(MSassets$moffen)
  set_BAR=rbind(assets$MSkabels[ValidatieSet][!is.na(Bronsysteem)],assets$MSmoffen[ValidatieSet][!is.na(Bronsysteem)],
                assets$LSkabels[ValidatieSet][!is.na(Bronsysteem)],assets$KSmoffen[ValidatieSet][!is.na(Bronsysteem)],fill=TRUE)
  save(set,file=paste0(settings$Analyse_Datasets,"/2. Proxy validatie/All_Asset_NOR_Info_Validatie.Rda"))
}
