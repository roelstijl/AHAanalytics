...
map = list()
if (!exists("assets$moffen")){
load(paste0(settings$Input_Datasets,"/1. AID KID proxy/AHA_Proxy_partial_data.Rda"),envir=globalenv())
}

# Data prep assets$kabels------------------------------
kabels_geo = assets$kabels[!is.na(assets$kabels$Coo_X_van) & 
                             pmatch(as.numeric(substring(assets$kabels$PC_XY_van,1,4)),PC_4,nomatch=0,duplicates.ok=TRUE)>0,]
lst=list()
...
# Data prep assets$moffen
assets$moffen[assets$moffen$DateRemoved=="","Change"] = "Added"
assets$moffen[!assets$moffen$DateRemoved=="","Change"] = "Removed"
assets$moffen$PC_4=substring(assets$moffen$PC_XY,1,4)
moffen_geo = assets$moffen[!is.na(assets$moffen$Coo_X) & pmatch(as.numeric(substring(assets$moffen$PC_XY,1,4)),PC_4,nomatch=0,duplicates.ok=TRUE)>0,]
