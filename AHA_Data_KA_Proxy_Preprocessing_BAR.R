AHA_Data_KA_Proxy_Preprocessing_BAR  = function()
  {
# Settings ----------------------------------------------------------------  
...
assets$MSHLD    = (mindataset)

load(paste0(settings$Ruwe_Datasets,"/1. BARlog/MH_NRG_LS_HLD.Rda"))
assets$LSHLD = (mindataset)

# Kabels --------------------------------------
cat("Processing Kabel data\n")
load(paste0(settings$Ruwe_Datasets,"/1. BARlog/MH_NRG_MS_KABELS_beginend.Rda"))
assets$MSkabels = (mindataset)

rbind(assets$MSkabels,AHA_Data_BAR_GEOMETRY(assets$MSkabels$Ligging,"beginend"))
assets$MSkabels[,Ligging:=NULL]

# load(paste0(settings$Ruwe_Datasets,"/1. BARlog/MH_NRG_LS_KABELS_beginend.Rda"))
...
# Moffen------------------------------ 
cat("Processing mof data\n")
load(paste0(settings$Ruwe_Datasets,"/1. BARlog/MH_NRG_LS_MOFFEN_position.Rda"))
assets$LSmoffen = mindataset
rbind(assets$LSmoffen,AHA_Data_BAR_GEOMETRY(assets$LSmoffen$Ligging,"position"))

load(paste0(settings$Ruwe_Datasets,"/1. BARlog/MH_NRG_MS_MOFFEN_position.Rda"))
