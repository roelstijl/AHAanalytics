AHA_Data_BAR_Log  = function(temp=FALSE)
{
...
if(temp){assets = list(); 
  changes = list();
  pb = tkProgressBar(title = paste0("AHA_Data_BAR_Log_Postprocessing, ",as.character(Sys.time())), label = "Start", min = 0, max = 8, initial = 0, width = 450);
  ...
  #print(object.size(assets))
  setTkProgressBar(pb, 6,label = "Toevoegen overige gegevens")
  # Bereken postcode 4
  ...
  ; cat("Saving to file\n"); 
  setTkProgressBar(pb, 7,label = "Saving to file")
  #print(object.size(assets))
  
  try(save(assets,file=paste0(settings$Input_Datasets,"/2. All Assets/Asset_Data_BAR_assets.Rda"),compress=F))
  #try(save(assets,file="Sinterklaas/Asset Health Analytics/2. All Assets/Asset_Data_BAR_assets_LSkabels.Rda",compress=F))
  #try(save(assets$LSmoffen,file="Sinterklaas/Asset Health Analytics/2. All Assets/Asset_Data_BAR_assets_LSmoffen.Rda",compress=F))
  #try(save(assets$MSkabels,file="Sinterklaas/Asset Health Analytics/2. All Assets/Asset_Data_BAR_assets_MSkabels.Rda",compress=F))
  #try(save(assets$MSmoffen,file="Sinterklaas/Asset Health Analytics/2. All Assets/Asset_Data_BAR_assets_MSmoffen.Rda",compress=F))
  all_ID_NAN = lapply(assets,function(x) try(unique(x$ID_NAN)))
  save(all_ID_NAN,file=paste0(settings$Input_Datasets,"/2. All Assets/Asset_Data_BAR_all_ID_NAN.Rda"))
  
  setTkProgressBar(pb, 8,label = "Done!")
